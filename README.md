# Unnati – AI‑Powered Personalized Learning Platform  
...
*A self‑hosted, LLM‑driven course generator that adapts to every learner.*

---

## 📖 Project Description
**Unnati** is an open‑source learning platform that uses local large‑language models (LLMs) to build **tailor‑made courses on demand**. Learners take a short pre‑assessment; the app then:
...
Originally developed for the **Intel® Unnati Program**, Unnati showcases how on‑premise generative AI can enhance education while preserving privacy and controlling cost. The reference implementation runs comfortably on a consumer‑grade **NVIDIA RTX 3050** GPU, but will scale down to CPU‑only laptops (with slower generation times) or up to data‑center GPUs for real‑time output.
